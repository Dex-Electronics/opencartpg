...
class ModelCatalogCategory extends Model {
	...
	public function getCategory($category_id) {
		...
		$cat = (int)$category_id;
		if($this->db->name == 'postgre') {
			$sql = <<<SQL
SELECT DISTINCT *, array_to_string(array(
                     SELECT cd1.name
                     FROM {$p}category_path cp
                     LEFT JOIN {$p}category_description cd1 ON (cd1.category_id = cp.path_id AND cp.category_id != cp.path_id)
                     WHERE cp.category_id = c.category_id AND cd1.language_id = '$lang'
                     GROUP BY cp.category_id, cd1.name, level
                     ORDER BY level), ' &gt; ') AS path,
                  (SELECT keyword
                   FROM {$p}url_alias
                   WHERE query = 'category_id=$cat') AS keyword
FROM {$p}category c
LEFT JOIN {$p}category_description cd2 ON (c.category_id = cd2.category_id)
WHERE c.category_id = '$cat' AND cd2.language_id = '$lang';
SQL;
		} else {
	...
	public function getCategories($data) {
		...
		$lang = (int)$this->config->get('config_language_id');
		if($this->db->name == 'postgre') {
		  $filter = empty($data['filter_name']) ? '' : " AND EXISTS (SELECT * FROM {$p}category_path cp JOIN {$p}category_description cd ON (cd.category_id = cp.category_id) WHERE cp.path_id = c.category_id AND cd.name ILIKE '" . $this->db->escape($data['filter_name']) . "%')";
			// PostgreSQL's array_agg feature does not have an option to sort results, as mysql's GROUP_CONCAT do
			$sql = <<<SQL
SELECT * FROM (
  SELECT c.category_id AS category_id,
         array_to_string(array(
           ...
           JOIN oc_category_description cd ON (cd.category_id = cp.path_id AND cd.language_id = '$lang')
           WHERE cp.category_id = c.category_id
           ORDER BY cp.level
         ), ' &gt; ') AS name,
         c.parent_id, c.sort_order
  FROM oc_category c
  WHERE EXISTS (SELECT * FROM oc_category_description WHERE category_id = c.category_id AND language_id = '$lang')
  $filter
) a ORDER BY name
SQL;
		} else {
