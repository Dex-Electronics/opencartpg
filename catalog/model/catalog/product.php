...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		$sql .= " GROUP BY p.product_id, p.sort_order, pd.name";
	...
	public function getProductSpecials($data = array()) {
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
				$sort = " ORDER BY LCASE(" . $data['sort'] . ")";
			} else {
				$sort = " ORDER BY " . $data['sort'];
			}
		} else {
			$sort = " ORDER BY p.sort_order";	
		}
		
		if (isset($data['order']) && ($data['order'] == 'DESC')) {
			$sort = " DESC, LCASE(pd.name) DESC";
		} else {
			$sort = " ASC, LCASE(pd.name) ASC";
		}
		...
		if (isset($data['start']) || isset($data['limit'])) {
		  ...
		  if($this->db->name == 'postgre') {
			  $sort = " LIMIT " . (int)$data['limit'] . " OFFSET " . (int)$data['start'];
			} else {
...
FROM (
  ...
  FROM {$p}product_special ps
  LEFT JOIN {$p}product p ON (ps.product_id = p.product_id)
  ...
  WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '$store' AND
        ps.customer_group_id = '$cgroup' AND
			  ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW()))
  GROUP BY ps.product_id $sort
) ps
SQL;
    } else {
