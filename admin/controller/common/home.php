		...
		switch ($range) {
			case 'day':
				for ($i = 0; $i < 24; $i++) {
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND (DATE(date_added) = DATE(NOW()) AND date_part('hour', date_added) = '" . (int)$i . "') GROUP BY date_part('hour', date_added)");
					...
					// count(*) and ORDER BY ?!
					$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE DATE(date_added) = DATE(NOW()) AND date_part('hour', date_added) = '" . (int)$i . "' GROUP BY date_part('hour', date_added)");
			...
			case 'month':
				for ($i = 1; $i <= date('t'); $i++) {
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND (DATE(date_added) = '" . $this->db->escape($date) . "') GROUP BY date_part('day', date_added)");
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE DATE(date_added) = '" . $this->db->escape($date) . "' GROUP BY date_part('day', date_added)");
			...
			case 'year':
				for ($i = 1; $i <= 12; $i++) {
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND date_part('year', date_added) = '" . date('Y') . "' AND date_part('month', date_added) = '" . $i . "' GROUP BY date_part('month', date_added)");
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE date_part('year', date_added) = '" . date('Y') . "' AND date_part('month', date_added) = '" . $i . "' GROUP BY date_part('month', date_added)");
